...
public interface ChatContentsRepository extends JpaRepository<ChatContents, Long> {
    @Query("SELECT c FROM ChatContents c WHERE c.court.id = :courtId AND c.id >= :lastId ORDER BY c.createdAt ASC")
    ...
    @Query("SELECT c FROM ChatContents c WHERE c.court.id = :courtId ORDER BY c.createdAt ASC")
